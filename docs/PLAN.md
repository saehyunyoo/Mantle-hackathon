...
- **메인넷 실배포:** **1개 시장** (선정 후보: 나스닥 — 글로벌 인지도 + 데이터 풍부)
- **테스트넷 + Mock:** 나머지 시장 (코스닥, 일본 TSE) — 데모 시각화용

### 7.2 토큰 수

- 매일 메인넷 10개 + 테스트넷 20개 = **약 30개/일** 토큰 발행 (시연 가능 규모)
...
| **W2** | 5/22 ~ 5/29 | 백엔드 (스냅샷 잡, Pyth 연동, Polygon.io 크롤링, Supabase 스키마) + 컨트랙트 메인넷 first deploy |
...
| Mantle RWA infrastructure | Pyth 오라클 + Mantle 메인넷 배포 |
...
| 리스크 | 완화책 |
|---|---|
| 메인넷 가스비 | 시장 1개로 한정, 가스 비싼 시간대 회피 |
| 오라클 시세 신뢰성 | Pyth 메인넷 우선, 폴백으로 Chainlink |
